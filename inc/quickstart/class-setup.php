...
class Setup extends \Smart_Plugin {
	 ...
	 *
	 * @since 1.13.1 Add use of Tool::save_metadata(), remove isset() check.
	 * @since 1.12.0 Updated to use external handle_shorthand().
	...
	protected function _save_meta_box( $post_id, $meta_box, $args ) {
		...
		if ( isset( $args['save_fields'] ) ) {
			...
			foreach ( $args['save_fields'] as $meta_key => $field ) {
				...
				Tools::save_meta_data( 'post', $post_id, $field, $meta_key, $args );
			}
		} else
		if ( isset( $args['callback'] ) || ( isset( $args['fields'] ) && is_callable( $args['fields'] ) ) ) {
			// Method 3: If generated by callback, attempt to save $_POST[$meta_box] if present
			Tools::save_meta_data( 'post', $post_id, $meta_box, $meta_box, $args );
		} else
		if ( isset( $args['fields'] ) || isset( $args['get_fields'] ) ) {
