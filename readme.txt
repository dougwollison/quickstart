...
= 1.10.0 =
Added term meta data support, along with custom fields for terms (as well as users); as such, the order manager feature now works for posts and terms. New wpedit helper now available, for inserting "Edit This" buttons onto the front end. Also added specialized handling of "hidden" input type, blanket "all" option available for load_helpers, and hide_comments now forcibly disables comments, pings, and XML RPC. Numerous but fixes, internal structure enhancements, and some additional utilities have been added as well.
